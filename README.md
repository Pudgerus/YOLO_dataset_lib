...
Разбивает датасет (пары изображение+метка) на подмножества: `train`, `val`, `test` в зависимости от выбора пользователя по заданным долям.

**Параметры:**

* `pairs` — список пар `(image_path, label_path)`.
* `ratios` — доли выборки, например `(0.7, 0.2, 0.1)`.

**Возвращает:**
Три списка пар для train/val/test.
